...
interface OnlineDrawingCanvasProps {
  isDrawer: boolean
  drawingData: DrawingPoint[]
  onDrawingChange?: (points: DrawingPoint[]) => void
}

export function OnlineDrawingCanvas({ isDrawer, drawingData, onDrawingChange }: OnlineDrawingCanvasProps) {
  ...
  // Send drawing data to server when local points change
  useEffect(() => {
  ...
  // Use server drawing data when not drawing
  ...
  // Reset local drawing when new turn starts
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col items-center space-y-4">
      ...
      {!isDrawer && <p className="text-sm text-gray-600">Watch the drawing and make your guess in the chat!</p>}
    </div>
  )
}
